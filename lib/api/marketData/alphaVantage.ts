...
const ALPHA_KEY = process.env.EXPO_PUBLIC_ALPHA_VANTAGE_API_KEY

export class AlphaVantageAdapter implements CandleProvider {
  name = 'alphaVantage'
  priority = 4
  ...
  async fetchCandles(symbol: string, timeframe: Timeframe, limit: number): Promise<MarketCandle[] | null> {
    ...
    try {
      ...
      const params = new URLSearchParams({
        ...
        from_symbol: alphaSymbol.split('-')[0],
        to_symbol: alphaSymbol.split('-')[1],
      ...
      if (data['Note'] || data['Information'] || data.error) return null
  ...
  private getFunctionName(interval: string): string {
    switch (interval) {
      case '1min': return 'FX_INTRADAY'
      case '5min': return 'FX_INTRADAY'
      case '15min': return 'FX_INTRADAY'
      case '30min': return 'FX_INTRADAY'
      case '1h': return 'FX_INTRADAY'
      case '2h': return 'FX_INTRADAY'
      case '4h': return 'FX_INTRADAY'
      case '8h': return 'FX_INTRADAY'
  ...
  private getIntervalParam(interval: string): string {
    switch (interval) {
      ...
      case '2h': return '60min'
      case '4h': return '60min'
      case '8h': return '60min'
